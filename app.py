"""
Cloudflare JS Challenge 绕过代理 - curl 版 (Hugging Face Space)
使用 curl 子进程避免 Python requests 的 TLS 指纹被 Cloudflare 阻断
"""
...
class SlowAES:
    ...
    @classmethod
    def _nr(cls, size): return {16: 10, 24: 12, 32: 14}[size]
    @classmethod
    def _expandKey(cls, key, size):
        nk, nr = size // 4, cls._nr(size)
        w = [0] * ((nr + 1) * 16)
        for i in range(nk * 4): w[i] = key[i]
        for i in range(nk, (nr + 1) * 4):
            ...
            w[b], w[b+1], w[b+2], w[b+3] = w[(i-nk)*4] ^ t0, w[(i-nk)*4+1] ^ t1, w[(i-nk)*4+2] ^ t2, w[(i-nk)*4+3] ^ t3
        return w
    @classmethod
    def _gmul(cls, a, b):
        p = 0
    ...
    def decrypt_cbc(cls, ct, key, iv):
        size, nr = len(key), cls._nr(len(key))
        w = cls._expandKey(key, size)
        pt, prev = [], list(iv)
        for i in range(0, len(ct), 16):
            state = ct[i:i+16][:]
            for j in range(16): state[j] ^= w[nr*16 + j]
            for rnd in range(nr-1, 0, -1):
                cls._invShiftRow(state)
                for j in range(16): state[j] = cls.rsbox[state[j]]
                for j in range(16): state[j] ^= w[rnd*16 + j]
                for c in range(4): cls._invMixCol(state[c*4:c*4+4])
            cls._invShiftRow(state)
            for j in range(16): state[j] = cls.rsbox[state[j]]
            for j in range(16): state[j] ^= w[j]
            for j in range(16): pt.append(state[j] ^ prev[j])
            prev = ct[i:i+16]
        if pt:
            pad = pt[-1]
...
def _to_nums(s): return [int(s[i:i+2], 16) for i in range(0, len(s), 2)]
def _to_hex(b): return ''.join(f'{x:02x}' for x in b)
...
def curl_post(url, data, cookie=None, timeout=20):
    """使用 curl 发送 POST 请求，返回 (http_code, body)"""
    cmd = [
        "curl", "-s", "-w", "\n%{http_code}", "-X", "POST",
        "-H", f"User-Agent: {UA}",
        ...
        "--connect-timeout", "10",
        "-d", data if isinstance(data, str) else "&".join(f"{k}={v}" for k, v in data.items()),
    ]
    if cookie:
        cmd.extend(["-H", f"Cookie: __test={cookie}"])
    cmd.append(url)
...
def post_with_bypass(path, data, retries=3):
    for attempt in range(retries):
        print(f"  [尝试 {attempt+1}] POST {path}")
        
        code, body = curl_post(f"{TARGET}{path}", data)
        print(f"  HTTP {code}, len={len(body)}")
        ...
        if "aes.js" in body or "slowAES" in body:
            ...
            if cv:
                print(f"  🔑 __test={cv}")
                time.sleep(1.5)
                code2, body2 = curl_post(f"{TARGET}{path}", data, cookie=cv)
                print(f"  HTTP {code2}, len={len(body2)}")
                if code2 == 0:
                    if attempt < retries - 1:
                        time.sleep(1)
                        continue
                    return 502, "curl timeout after challenge"
                if "aes.js" in body2 or "slowAES" in body2:
                    print(f"  ❌ 重试后仍是挑战页")
                    continue
                print(f"  ✅ 绕过成功!")
                return code2, body2
...
def proxy_notify():
    ...
    status, body = post_with_bypass('/mpayNotify', data)
    return Response(body, status=status, content_type='text/html; charset=utf-8')

@app.route('/<path:path>', methods=['POST', 'GET'])
def proxy_any(path):
    data = request.form if request.form else request.get_data(as_text=True)
    status, body = post_with_bypass(f"/{path}", data)
    return Response(body, status=status, content_type='text/html; charset=utf-8')
